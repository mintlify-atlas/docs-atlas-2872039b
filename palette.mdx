---
title: Palette
description: Complete color reference for all 26 colors across all 4 Catppuccin flavors, with hex, RGB, and HSL values.
---

Catppuccin defines 26 named colors that are consistent across all four flavors. The names never change — only the values differ between Latte, Frappé, Macchiato, and Mocha.

## Color roles

Colors are grouped into five semantic categories:

| Category | Colors | Typical use |
| --- | --- | --- |
| **Accent** | Rosewater, Flamingo, Pink, Mauve, Red, Maroon, Peach, Yellow, Green, Teal, Sky, Sapphire, Blue, Lavender | Syntax highlighting, semantic UI color |
| **Text** | Text, Subtext1, Subtext0 | Body copy, secondary and muted text |
| **Overlay** | Overlay2, Overlay1, Overlay0 | Comments, disabled elements, borders |
| **Surface** | Surface2, Surface1, Surface0 | Cards, input fields, raised UI elements |
| **Base** | Base, Mantle, Crust | Backgrounds |

<Info>
  All ports use the same color names. When building a port or library integration, reference colors by name (e.g., `mocha.blue`) rather than by hex value to stay flavor-agnostic.
</Info>
...
<Tabs>
  <Tab title="Latte">
    Latte is the light flavor. It uses dark text on a warm off-white base.
  ...
  <Tab title="Frappé">
    Frappé is the lightest of the three dark flavors, using muted blue-gray tones.
  ...
  <Tab title="Macchiato">
    Macchiato is the medium dark flavor, balancing contrast and low-light comfort.
  ...
  <Tab title="Mocha">
    Mocha is the darkest flavor, using deep navy-black tones for low-light environments.
...
For programmatic access to palette values, the official [catppuccin/palette](https://github.com/catppuccin/palette) repository provides integrations for popular frameworks and languages. Language-specific libraries are also available for Rust, Python, Go, Java, Flutter, and more — see the [ports list](https://github.com/catppuccin/catppuccin#-ports-and-more) for the full set.

<Tip>
  Before building a port, read the [style guide](/style-guide) for guidance on which colors to use for specific UI roles such as borders, selections, syntax keywords, and backgrounds.
</Tip>
